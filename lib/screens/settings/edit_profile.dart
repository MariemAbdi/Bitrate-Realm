...
class _EditProfileState extends State<EditProfile> {
  ...
  updateImage({bool isCover = false}) async {
    ...
    if (pickedImage != null) {
      ...
      try {
        if(!mounted) return;
        if(isCover) {
          await context.read<UserProvider>().updateCoverImage(pickedImage, firebaseAuthUser.email!);
        }else{
  ...
  removeImage({bool isCover = false}) async {
    Get.back(); // Hide bottom sheet
    showLoadingPopUp();
    try {
      if(isCover){
        await context.read<UserProvider>().removeCoverImage(firebaseAuthUser.email!);
      }else{
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  EditProfileImage(
                    isProfile: false,
                    profileImageLink: user?.coverURL,
                    updateImage: ()=>updateImage(isCover: true),
                    removeImage: ()=>removeImage(isCover: true),
                  ),
                ],
              ),
              kVerticalSpace,
