...
class BroadcastScreen extends StatefulWidget{
  ...
  const BroadcastScreen({Key? key, required this.isBroadcaster, this.liveStream}) : super(key: key);
...
class _BroadcastScreenState extends State<BroadcastScreen> with WidgetsBindingObserver{
  ...
  Future<void> deleteDocument() async {
    //await LiveStreamServices().endLiveStream(widget.liveStream?.channelId ?? "", user);
  }
  ...
  Future<void> getToken() async {
    final res = await http.get(
      Uri.parse('${dotenv.env["RENDER_BASEURL"]}/rtc/${widget.liveStream?.channelId ?? ""}/publisher/userAccount/${
  ...
  void _joinChannel() async{
    ...
    if(token!=null){
      ...
      //if(!mounted)return;
      await _engine.joinChannelWithUserAccount(token, widget.liveStream?.channelId ?? "",
         FirebaseAuthServices().user!.uid
         //context.read<FirebaseAuthServices>().user!.uid
  ...
  Widget build(BuildContext context) {
    ...
    _leaveChannel(String uid) async{
      ...
      if(uid == (widget.liveStream?.channelId ?? "")){
        await LiveStreamServices().endLiveStream(widget.liveStream?.channelId ?? "", user!);
        _engine.disableAudio();
        _engine.disableVideo();
      }else{
        //IF THE ONE LEAVING IS A VIEWER THEN THE VIEWS COUNT IS UPDATED
        await LiveStreamServices().updateViewCount(widget.liveStream?.channelId ?? "", false);
      }
                    ...
                    children: [
                      ...
                      Visibility(
                        ...
                        child: Container(
                          ...
                          child: Expanded(
                              child: Chat(collectionName: 'live_streams', docId: widget.liveStream?.channelId ?? "")
                          ),
                        ),
                      )
                    ],
  ...
  _renderVideo(isScreenSharing) {
    return AspectRatio(
      ...
      child: widget.isBroadcaster//"${user.uid}" == widget.channelId
          ...
          ? kIsWeb
          ? rtc_remote_view.SurfaceView(uid: _remoteUid[0], channelId: widget.liveStream?.channelId ?? "", mirrorMode: VideoMirrorMode.Auto, renderMode: VideoRenderMode.Hidden,)
          : rtc_remote_view.TextureView(uid: _remoteUid[0], channelId: widget.liveStream?.channelId ?? "")
          : Container(),
    );
  }

  void getViewCount() {
    FirebaseFirestore.instance.collection("live_streams").doc(widget.liveStream?.channelId).get().then((DocumentSnapshot documentSnapshot) {
      setState(() {
