...
class BroadcastScreen extends StatefulWidget{
  ...
  final LiveStream? liveStream; //USED TO SHOW THE LIVE'S INFO
...
class _BroadcastScreenState extends State<BroadcastScreen> with WidgetsBindingObserver{
  ...
  Future<void> deleteDocument() async {
    await endLiveStream(widget.channelId);
  }
  ...
  void initState() {
    ...
    Timer.periodic(const Duration(seconds: 1), (timer) {
      setState(() {
        elapsedTime = DateTime.now().difference(widget.liveStream!.date).inSeconds;
  ...
  Future<void> getToken() async {
    final res = await http.get(
      Uri.parse('${dotenv.env["RENDER_BASEURL"]}/rtc/${widget.channelId}/publisher/userAccount/${context.read<FirebaseAuthMethods>().user.uid}/'),
    );
  ...
  void _joinChannel() async{
    ...
    if(token!=null){
      ...
      //if(!mounted)return;
      await _engine.joinChannelWithUserAccount(token, widget.channelId, context.read<FirebaseAuthMethods>().user.uid);
    }
  }
  ...
  _leaveChannel(String uid) async{
    ...
    if(uid == widget.channelId){
      ...
      await updateViewCount(widget.channelId, false);
    }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        customConfirmationCoolAlert(context, LocaleKeys.leave.tr(), LocaleKeys.areYouSureYouWantToLeave.tr(), "assets/lottie/sad-dog.json", ()async {await _leaveChannel(context.read<FirebaseAuthMethods>().user.uid);});
        ...
        Visibility(
          ...
          child: ConstrainedBox(
            ...
            child: Padding(
              ...
              child: CustomButton(text: LocaleKeys.leave.tr(), function: (){
                customConfirmationCoolAlert(context, LocaleKeys.leave.tr(), LocaleKeys.areYouSureYouWantToLeave.tr(), "assets/lottie/sad-dog.json",
                  ()async {
                  await _leaveChannel(context.read<FirebaseAuthMethods>().user.uid);
                });
              },),
            ),
          ),
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text(DateFormat('dd/MM/yyyy HH:mm').format(widget.liveStream!.date),style: GoogleFonts.ptSans(textStyle: const TextStyle(fontWeight: FontWeight.w400, fontSize: 14))),
                                  ],
                                ),
                              ],
                            ),
